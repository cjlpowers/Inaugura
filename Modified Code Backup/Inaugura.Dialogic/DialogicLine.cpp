...
{	
	...
	void DialogicLine::ClearDigitsBuffer()
	{
		try
		{
			mpDialogicLine->ClearDigitsBuffer();
		}
		catch(char* str)
		{
			DialogicException __gc *ex = new DialogicException(str);
	...
	void DialogicLine::RecordWav(String __gc* fileName)
	{
		try
		{
			mpDialogicLine->RecordFile(fileName);
		}
		catch(char* str)
		{
			CString str2 = str;
