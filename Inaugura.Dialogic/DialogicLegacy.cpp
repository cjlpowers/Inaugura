...
int DialogicLegacy::OpenLine()
{
	...
	*/
	//if ( dx_blddt( TID_1, 440, 25, 480, 25, TN_LEADING )== -1 ) 
	//	throw "Unable to build a Dual Tone Template";
	...
	* 1050 Hz and 475-525 Hz using leading edge detection.
	*/
	//if ( dx_blddt( TID_2, 440, 25, 480, 25, TN_TRAILING )== -1 ) 
	//	throw "Unable to build a Dual Tone Template";
	/*
	* Add the Tone to the Channel
	*/
	//if ( dx_addtone( mChannelHandle, NULL, 0 ) == -1 ) 
	//	throw ATDV_ERRMSGP(mChannelHandle);		
...
void DialogicLegacy::ClearDigitBuffer()
{
...
CString DialogicLegacy::GetDigits( int maxDigits, float timeOutSec, CString termDigits, float digitTimeOutSec, float silenceTimeOutSec, CString& termDigit)
{
	...
	int timeOutMS = int(timeOutSec * _100MS_PER_SEC);
	int digitTimeOutMS = int(digitTimeOutSec * _100MS_PER_SEC);
	int silenceTimeOutMS = int(silenceTimeOutSec * _100MS_PER_SEC);

	try
	{

		if(timeOutMS == 0 && timeOutSec > 0) // check for loss in precision
			timeOutMS = 1;
		if(digitTimeOutMS == 0 && digitTimeOutSec > 0) // check for loss in precision
			digitTimeOutMS = 1;
		if(silenceTimeOutMS == 0 && silenceTimeOutSec > 0) // check for loss in precision
			silenceTimeOutMS = 1;
		...
		if(timeOutMS > 0) // watch for time out
		{		
			// create the TPT structure
			tpt[optionIndex].tp_type	= IO_CONT;
			tpt[optionIndex].tp_termno	= DX_MAXTIME;
			tpt[optionIndex].tp_length	= timeOutMS;
			tpt[optionIndex].tp_flags	= TF_MAXTIME;
			optionIndex++;
		}
		if(digitTimeOutMS > 0) // watch for digit time out
		{				
			// create the TPT structure
			tpt[optionIndex].tp_type	= IO_CONT;
			tpt[optionIndex].tp_termno	= DX_IDDTIME;
			tpt[optionIndex].tp_length	= digitTimeOutMS;
			tpt[optionIndex].tp_flags	= TF_IDDTIME;
			optionIndex++;
		}
		if(silenceTimeOutMS > 0) // watch for max digits
		{		
			// create the TPT structure
			tpt[optionIndex].tp_type	= IO_CONT;
			tpt[optionIndex].tp_termno	= DX_MAXSIL;
			tpt[optionIndex].tp_length	= silenceTimeOutMS;
	...
	catch(...)
	{
		...
		throw "Unknown Error in GetDigits";
	}	
	return returnstring;
}

void DialogicLegacy::LoadTone(unsigned int toneID, unsigned int userToneID)
{
	TN_GENCAD *tone = dx_get_tngencad(toneID);

	// Create the tone
	if ( dx_blddt( userToneID, tone->tone[0].tg_freq1, 25, tone->tone[0].tg_freq2, 25, TN_LEADING )== -1 ) 
		throw ATDV_ERRMSGP(mChannelHandle);
	...
	if( dx_enbtone(mChannelHandle,userToneID, DM_TONEON | DM_TONEOFF) == -1)
		throw ATDV_ERRMSGP(mChannelHandle);
}

int DialogicLegacy::DetectRingback(float timeOutSec)
{
	const static float _100MS_PER_SEC = 10;	
	int optionsCount =0;
	int optionIndex = 0;
	...
	// enable the tone
	if( dx_enbtone(mChannelHandle,TID_1, DM_TONEON | DM_TONEOFF) == -1)
		throw ATDV_ERRMSGP(mChannelHandle);
	*/
	...
	LoadTone(9,TID_4);
		...
		// create a new tpt struct with enough space to hold all the options
		tpt = new DV_TPT[optionsCount+9];	// +1 since we must also detect tones
		dx_clrtpt(tpt,optionsCount+9);
		if(timeOutMS > 0) // watch for time out
		{		
			// create the TPT structure
			tpt[optionIndex].tp_type	= IO_CONT;
		...
		// we always want to handle Ring Tone
		tpt[optionIndex].tp_type	= IO_CONT;	
		tpt[optionIndex].tp_termno	= DX_TONE;	
		tpt[optionIndex].tp_length	= TID_1;  // ring back tone 1
		tpt[optionIndex].tp_flags	= TF_TONE;	
		tpt[optionIndex].tp_data	= DX_TONEON;
		optionIndex++;
		...
		tpt[optionIndex].tp_length	= TID_1;  // ring back tone 1
		tpt[optionIndex].tp_flags	= TF_TONE;	
		tpt[optionIndex].tp_data	= DX_TONEOFF;
		optionIndex++;

		// we always want to handle Ring Tone
		tpt[optionIndex].tp_type	= IO_CONT;	
		tpt[optionIndex].tp_termno	= DX_TONE;	
		tpt[optionIndex].tp_length	= TID_2;  // ring back tone 1
		tpt[optionIndex].tp_flags	= TF_TONE;	
		tpt[optionIndex].tp_data	= DX_TONEON;
		optionIndex++;
		...
		tpt[optionIndex].tp_type	= IO_CONT;	
		tpt[optionIndex].tp_termno	= DX_TONE;	
		tpt[optionIndex].tp_length	= TID_3;  // ring back tone 1
		tpt[optionIndex].tp_flags	= TF_TONE;	
		tpt[optionIndex].tp_data	= DX_TONEON;
		optionIndex++;
		...
		tpt[optionIndex].tp_type	= IO_CONT;	
		tpt[optionIndex].tp_termno	= DX_TONE;	
		tpt[optionIndex].tp_length	= TID_4;  // ring back tone 1
		tpt[optionIndex].tp_flags	= TF_TONE;	
		tpt[optionIndex].tp_data	= DX_TONEON;
		optionIndex++;
		...
		if(term & TM_TONE) //terminated because specified digit received
		{
			return TM_TONE;
		}		
		else if(term & TM_LCOFF) //customer hungup
	...
	catch(...)
	{
		...
		throw "Unknown Error in DetectRingback";
	}	
	return 0;
}

int DialogicLegacy::PlaySpecial(CString Filename)
{	
	...
	LoadTone(1,TID_4);				
		...
		// create a new tpt struct with enough space to hold all the options
		tpt = new DV_TPT[9];	// +1 since we must also detect tones
		dx_clrtpt(tpt,9);		
...
{	
	...
	if(TermDigits != "")
	{
		...
		else // dont terminate on just any key
		{
			...
			//set up DV_TPT with a termmask
			DV_TPT tpt[] = {{IO_CONT, DX_LCOFF, 1, TF_LCOFF},
			{IO_EOT,  DX_DIGMASK, TermMaskDigit, TF_DIGMASK}};
	...
	else
	{
		...
		{
			//set up DV_TPT without a termmask
			DV_TPT tpt[] = {{IO_EOT, DX_LCOFF, 1, TF_LCOFF}};
	...
	term = ATDX_TERMMSK(mChannelHandle);

	/* TODO REMOVE THIS DEBUG CODE
	FILE *fp;
	fp = fopen("PlayFile.log","at");
	if(fp)
	{
		fprintf(fp,"Channel Handle [%d] File [%s] Terminate [%d] TermDigits [%s] Result [%d]\n",mChannelHandle,Filename,sTerminate,TermDigits,term);
		fclose(fp);
	}
	// END TODO */
...
int DialogicLegacy::RecordFile(CString Filename, float timeOutSec, float silenceTimeOutSec, int terminate)
{
	const static float _100MS_PER_SEC = 10;
	CString	Tracestring;
	long term;
	...
	if(silenceTimeOutMS > 0) // was silenceTimeOut specified (optional)
	{
		if(terminate == 1)
		{
			DV_TPT tpt[] = {{IO_CONT, DX_MAXDTMF, 1, TF_MAXDTMF},
			{IO_CONT, DX_LCOFF, 1, TF_LCOFF},
			{IO_CONT, DX_MAXTIME, timeOutMS, TF_MAXTIME},
			{IO_EOT,  DX_MAXSIL, silenceTimeOutMS , TF_MAXSIL|TF_SETINIT,40}};
			
			//now record the file
			if (dx_recwav(mChannelHandle,Filename,tpt, &xpb,PM_TONE|EV_SYNC) == -1)
				throw ATDV_ERRMSGP(mChannelHandle);				
		}
		else // no digit termination
		{
			DV_TPT tpt[] = {{IO_CONT, DX_LCOFF, 1, TF_LCOFF},
			{IO_CONT, DX_MAXTIME, timeOutMS, TF_MAXTIME},
			{IO_EOT,  DX_MAXSIL, silenceTimeOutMS , TF_MAXSIL|TF_SETINIT,40}};
	...
	else
	{
		if(terminate == 1)
		{
			...
			DV_TPT tpt[] = {{IO_CONT, DX_MAXDTMF, 1, TF_MAXDTMF},
			{IO_CONT, DX_LCOFF, 1, TF_LCOFF},
			{IO_EOT, DX_MAXTIME, timeOutMS, TF_MAXTIME}};
...
int DialogicLegacy::GetBoardCount()
{
    ...
    if ( sr_getboardcnt(DEV_CLASS_VOICE, &numvoxbrds) == -1)
        { 
			...
			throw "Unable to retreive the number of voice boards installed on the system";            
        }
