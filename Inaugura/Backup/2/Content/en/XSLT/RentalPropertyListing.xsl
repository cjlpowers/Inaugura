...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:if test ="count(Images/Image) > 1">
      <div class="photos">
        <xsl:for-each select="Images/Image">
          <span class="photo">
            <img  src="ImageHandler.ashx?id={@fileId}&amp;mode=160" title="{Description/text()}" onclick="javascript:SetImage('{@fileId}')"/>
          </span>
        </xsl:for-each>
      </div>
    </xsl:if>
  ...
  <xsl:template name="Map">
    <input type="hidden" name="_listingID" id="_listingID" value="{@id}" />
    <a onclick="ShowListingMapWithPin('{Address/@latitude}','{Address/@longitude}','{Title/text()}','')" class="linkMap" title="See on Map">
      <span class="address">
    ...
    <div id="listingMap" style="position:relative; width:100%; height:450px;"></div>
  </xsl:template>
  ...
  <xsl:template name="Details">
      <div class="details">        
            <xsl:choose>
              <xsl:when test ="count(Images/Image) > 0">
                <div style="float:left; padding:0px 10px 10px 0px;">
                  <img id="listingImage2"  class="Photo" onclick="changeTab(1);">
                <xsl:variable name="displayImage">
                  <xsl:value-of select="Images/@defaultImage"/>
                </xsl:variable>
                <xsl:attribute name="src">
              ...
              <!--<xsl:otherwise>
                <xsl:attribute name="src">
                  <xsl:text>ImageHandler.ashx?id=noimage&amp;mode=320</xsl:text>
                </xsl:attribute>
              </xsl:otherwise>-->
            </xsl:choose>
        ...
        <xsl:if test="$admin='True'">
          <div class="admin">
            <a class="adminTask"  href="javascript:FireEvent('edit,~/Controls/Edit/EditAddress.ascx;listing,{@id}')">Change Address</a>
          </div>
        </xsl:if>
        <span class="address">
          <font class="highlight">
        ...
        <br/>
        <span >
          <font class="label">Distance: </font>
          <font class="highlight">
            <xsl:call-template name="Distance"></xsl:call-template>
          </font>
        </span>
        ...
        <xsl:if test="$admin='True'">
          <div class="admin">
            <a class="adminTask"  href="javascript:FireEvent('edit,~/Controls/Edit/EditRentalDetails.ascx;listing,{@id}')">Change Details</a>
          </div>
        </xsl:if>
        <xsl:if test="@monthlyRent">
          <span class="large">
            <font class="label">Rent: </font>
            <font class="highlight">
              <xsl:text>$</xsl:text>
              <xsl:value-of select="format-number(@monthlyRent,'###,###,###')"/>
            </font>
            <font class="fine">/ month</font>
          </span>
        </xsl:if>
        <br/>
        <xsl:if test="@availabilityStart">
          <span >
            <font class="label">Availiable: </font>
            <font class="highlight">
              <xsl:value-of select="@availabilityStart"/>
            </font>
            <xsl:if test="@availabilityEnd">
              <font class="label"> until </font>
              <font class="highlight">
                <xsl:value-of select="@availabilityEnd"/>
              </font>
            </xsl:if>
        ...
        <xsl:if test="RentalPropertyType">
          <span class="detail">
            <font class="label">Property Type: </font>
            <font class="highlight">
              <xsl:apply-templates select="RentalPropertyType"/>
              <xsl:apply-templates select="FurnishingType"/>
            </font>
        ...
        <xsl:if test="@numberOfBedrooms > 0">
          <span>
            <font class="label">Bedrooms: </font>
            <font class="highlight">
        ...
        <xsl:if test="@numberOfBedrooms > 0">
          <span>
            <font class="label">Bathrooms: </font>
            <font class="highlight">
              <xsl:value-of select="@numberOfBathrooms"></xsl:value-of>
            </font>
          </span>
        </xsl:if>
        <xsl:if test="count(Levels/Level/Rooms/Room/FloorMaterial) > 0">
          <span>
        ...
        <xsl:if test="Lot/Size">
          <span >
            <font class="label">Lot Size: </font>
            <font class="highlight">
              <xsl:apply-templates select="Lot/Size"></xsl:apply-templates>
            </font>
          </span>
        </xsl:if>
        <xsl:if test="@parkingSpaces > 0">
          <span>
            <font class="label">Parking Spaces: </font>
            <font class="highlight">
              <xsl:value-of select="@parkingSpaces"/>
            </font>
            <xsl:if test="@parkingIncluded">
              <font class="fine">(included)</font>
            </xsl:if>
          </span>
        </xsl:if>
        <xsl:if test="Appliances">
          <span class="appliances">
            <font class="label">Appliances:</font>
            <font class="highlight">
              <xsl:value-of select="count(Appliances/Item)"></xsl:value-of>
            </font>
            <font class="fine">
              <xsl:text>( </xsl:text>
              <xsl:for-each select="Appliances/Item">
      ...
      <div class="description">        
        ...
        <xsl:if test="$admin='True'">
          <div class="admin">
            <a class="adminTask"  href="javascript:FireEvent('edit,~/Controls/Edit/EditDescription.ascx;listing,{@id}')">Change Description</a>
          </div>
        </xsl:if>
    ...
    <xsl:if test="$admin='True'">
      <a class="adminTask"  href="javascript:FireEvent('edit,~/Controls/Edit/EditLevel.ascx;listing,{@id}')">Add Level</a>
      <br/>
    </xsl:if>
      ...
      <xsl:if test="Lot">
        <div class="lot">
          ...
          <xsl:if test="$admin='True'">
            <div class="admin">
              <a class="adminTask"  href="javascript:FireEvent('edit,~/Controls/Edit/EditLot.ascx;listing,{@id}')">Edit Lot</a>
            </div>
          </xsl:if>
          ...
          <xsl:if test="Lot/Features">
            <h3>Features</h3>
            <xsl:apply-templates select="Lot/Features"></xsl:apply-templates>
          </xsl:if>
        </div>
      </xsl:if>
            ...
            <tr>
              <td colspan="4">
                ...
                <div>
                  ...
                  <div style="text-align:center">
                    <img id="listingImage3"  class="Photo">
                      <xsl:choose>
                        <xsl:when test ="count(Images/Image) > 0">
                          <xsl:variable name="displayImage">
                            <xsl:value-of select="Images/@defaultImage"/>
  ...
  <xsl:template name="Mini">
    <td>
      <table class="Mini" onclick="_ListingSelected('{@id}')">
        <!--onclick="GetListingContent('{@id}')">-->
        <tr>
          <th colspan="2">
            <span style="float:left">
  ...
  <xsl:template name="Pin">
    <div onclick="_ListingSelected('{@id}')">
      <span>
        <xsl:call-template name="HeaderIcons"/>
      </span>
      <xsl:call-template name="Distance"></xsl:call-template>
    ...
    <xsl:if test="count(Images/Image) > 0">
      <img id="featureImage" style="border: solid 1px #BBBBBB;cursor:pointer" onclick="_ListingSelected('{@id}')" >
        <xsl:variable name="displayImage">
          <xsl:value-of select="Images/@defaultImage"/>
        </xsl:variable>
        <xsl:attribute name="src">
    ...
    <a onclick="_ListingSelected('{@id}')" class="linkView" title="View Listing">View</a>
  </xsl:template>
